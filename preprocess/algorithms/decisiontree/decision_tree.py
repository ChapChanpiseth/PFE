...
from algorithms.decisiontree.tree import Tree
...
class DecisionTree(Base, Tree):
  ...
  def load_model(self):
    ...
    try:
      ...
      model = FileHelper.load_model(head+ '/decision_tree_' +tail)
  ...
  def save_model(self, model):
    ...
    try:
      head, tail = ntpath.split(self.kwargs['train_model'])
      FileHelper.save_model(head + '/decision_tree_' +tail, model.__dict__)
    except IOError as error:
      print(error)
